...
export default async function AssinaturaCartaoPage() {
  try {
    ...
    console.error('PUBLIC_CARD_PAGE_FAILED', error);
      ...
      <AuthShell headerLinkHref="/login" headerLinkLabel="Área do cliente" mode="single">
        <Card className="w-full border-slate-200 bg-white shadow-[0_18px_60px_rgba(15,23,42,0.08)]">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl text-slate-900">Não foi possível abrir o pagamento</CardTitle>
          ...
          <CardContent className="flex flex-col gap-3 sm:flex-row">
            ...
            <Button asChild className="flex-1 rounded-xl bg-slate-950 hover:bg-slate-800">
              <Link href="/assinatura/cartao">Tentar novamente</Link>
            </Button>
          </CardContent>
        </Card>
      </AuthShell>
