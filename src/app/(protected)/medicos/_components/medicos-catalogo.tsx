  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-white px-5 py-5 shadow-sm md:px-6">
        <h1 className="text-[1.85rem] font-semibold text-slate-900 md:text-[2rem]">Medicos</h1>
        <p className="mt-1 text-sm text-slate-500">Gerencie o corpo clinico, horarios, especialidades e dados de atendimento.</p>

        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500">Medicos cadastrados</p>
            <p className="mt-1 text-2xl font-semibold text-slate-800">{doctors.length}</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500">Especialidades ativas</p>
            <p className="mt-1 text-2xl font-semibold text-slate-800">{specialties.length}</p>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
          <div className="relative flex-1">
            ...
            <Input
              ...
              className="border-slate-200 bg-white pl-9 text-slate-700 placeholder:text-slate-400 shadow-none focus-visible:border-blue-300 focus-visible:ring-blue-200"
              placeholder="Buscar por nome, CRM ou especialidade"
            />
          </div>

          <div className="flex flex-wrap items-center gap-2 xl:flex-nowrap">
            <div className="relative flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-slate-50">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
        {filteredDoctors.map((doctor) => {
          ...
          return (
            <div key={doctor.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
              <div className="border-b border-slate-100 bg-slate-50/70 p-5">
                <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0 flex-1 self-center">
                    <h3 className="line-clamp-2 break-words text-[1.12rem] font-semibold leading-tight text-slate-900 [overflow-wrap:anywhere]">
                      {doctor.name}
                    </h3>
                    <div className="mt-2.5 space-y-2">
                      <div className="inline-flex max-w-full items-start gap-1.5 rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-medium text-emerald-700">
                        <Stethoscope className="mt-0.5 size-3 shrink-0" />
                        <span className="line-clamp-2 break-words leading-4">{doctor.specialty}</span>
                      </div>
                      <div>
                        <Badge variant="outline" className="rounded-full border-slate-200 px-2.5 py-1 text-[10px] font-medium text-slate-600">
                          CRM {doctor.crm}
                        </Badge>
                      </div>
                    </div>
