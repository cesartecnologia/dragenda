      ...
      <CardHeader className="space-y-4 border-b border-slate-100 pb-5">
        <div className="flex items-center justify-between gap-3">
          <div className="space-y-1.5">
            <CardTitle className="text-xl text-slate-900">Equipe em destaque</CardTitle>
            <div className="text-sm text-slate-500">Quem mais atendeu no período.</div>
          </div>
          <div className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-[#eef4ff] text-primary">
            <Activity className="size-5" />
          </div>
        </div>

        <div className="rounded-[22px] bg-[linear-gradient(135deg,#eef4ff_0%,#f8fbff_100%)] p-4">
      ...
      <CardContent className="space-y-4 px-5 py-5">
        {doctors.length ? (
          doctors.map((doctor) => {
            ...
            return (
              <div key={doctor.id} className="rounded-[22px] border border-slate-100 bg-white p-4 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_12px_22px_rgba(125,160,220,0.10)]">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-3">
                    <Avatar className="h-11 w-11 rounded-2xl border border-white bg-white shadow-[0_8px_16px_rgba(125,160,220,0.10)]">
                  ...
                  <div className="rounded-full bg-[#eef4ff] px-3 py-1 text-xs font-semibold text-slate-700">
                    {doctor.appointments}
                  </div>
                </div>
                <div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100">
                  <div className="h-full rounded-full bg-[linear-gradient(90deg,#8cb7ff,#6e85ca)]" style={{ width: `${progress}%` }} />
                </div>
              </div>
            );
          })
